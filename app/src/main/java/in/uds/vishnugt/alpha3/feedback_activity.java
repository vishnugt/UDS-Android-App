    ...
    private class LongOperationsubmit extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                ...
                oswrite=oswrite.concat("\"type\":\"feedback\",");
                oswrite=oswrite.concat("\"query\":\"attendance\",\"reply\":\""+input.get(0).toString()+"\"},");
                oswrite=oswrite.concat("{\"type\":\"feedback\",");
                oswrite=oswrite.concat("\"query\":\"grooming\",\"reply\":\""+input.get(1).toString()+"\"},");
                oswrite=oswrite.concat("{\"type\":\"feedback\",");
                oswrite=oswrite.concat("\"query\":\"cleanlinessToilet\",\"reply\":\""+input.get(2).toString()+"\"},");
                oswrite=oswrite.concat("{\"type\":\"feedback\",");
                oswrite=oswrite.concat("\"query\":\"cleanlinessFloor\",\"reply\":\""+input.get(3).toString()+"\"},");
                oswrite=oswrite.concat("{\"type\":\"feedback\",");
                oswrite=oswrite.concat("\"query\":\"cleanlinessPantry\",\"reply\":\""+input.get(4).toString()+"\"},");
                oswrite=oswrite.concat("{\"type\":\"feedback\",");
                oswrite=oswrite.concat("\"query\":\"cleanlinessDusting\",\"reply\":\""+input.get(5).toString()+"\"},");
                oswrite=oswrite.concat("{\"type\":\"feedback\",");
                oswrite=oswrite.concat("\"query\":\"feedback\",\"reply\":\""+input.get(6).toString()+"\"},");
                oswrite=oswrite.concat("{\"type\":\"feedback\",");
                oswrite=oswrite.concat("\"query\":\"comments\",\"reply\":\""+input.get(7).toString()+"\"}");
                Date today = new Date();
                String current = today.toString();
                oswrite=oswrite.concat("], \"requestFields\": { \"projectId\": \""+companyid+"\", \"projectDesc\": \""+desc+"\", \"recordcreationdate\": \""+current+"\", \"monthandYear\": \""+month+" "+year+"\", \"timeClient\" : \"\", \"timeRegional\" : \"\", \"clientId\" : \"\", \"regionalHeadId\" : \"\", \"date\" : \"" + date + " " + timeIn + " - " + timeOut + "\", \"deliveryOrVisit\": \"visit\" , \"companyName\":\""+client+"\"}, \"requestType\": \"supervisor\", \"fresh\": true, \"status\": \"Visited\", \"changed\": true, \"transitions\": { \"1\": \"Visited\" }, \"editReason\": \"Automated from app\" }");
